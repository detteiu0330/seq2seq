...
def load_data(vocabulary, path, device):
    ...
    with io.open(path, encoding='utf-8') as f:
        for line in bar(f):
            ...
            array = np.array([vocabulary.get(w, UNK) for w in words])
            if max_len < array.size:
                max_len = array.size
            data.append(array)
    return data, max_len
...
def main():
    ...
    parser.add_argument('--epoch', type=int, default=2, help='number of epoch of training')
    ...
    train_source, source_max_len = load_data(source_ids, args.SOURCE, args.device)
    train_target, target_max_len = load_data(target_ids, args.TARGET, args.device)
    ...
    bos = torch.zeros(1, dtype=torch.int64).to(args.device)
    eos = torch.ones(1, dtype=torch.int64).to(args.device)
    ...
    for i in range(args.epoch):
        ...
        for step, train_data in enumerate(train_data):
            ...
            sources = torch.from_numpy(sources).to(args.device)
            targets = torch.from_numpy(targets).to(args.device)
            print(sources)
            print(bos)
            encoder_optimizer.zero_grad()
            decoder_optimizer.zero_grad()
            xs = torch.cat((bos, targets), dim=0)
            ts = torch.cat((targets, eos), dim=0)
            hn = encoder(sources)
